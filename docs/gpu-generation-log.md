...
- Promoted `deepface-retinaface` into the normal backend registry, so `build`, `evaluate`, and `compare-backends` can use the audited RetinaFace detector path directly instead of requiring a detector sweep run.
- Smoke-verified the registered `deepface-retinaface` backend through `compare-backends` on `data/raw/seju_official_sample` vs `outputs/generated_detector_v5`: refs `2/2`, generated images `1/1`, best image `candidate_0001_seed_260623`, best score `0.293422`.
